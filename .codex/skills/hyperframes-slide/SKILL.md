...
4. **`overview.html` 동기화**
   ...
   - 16:9 슬라이드 overview에는 Full HD 내보내기 기능을 포함한다.
     - `PNG`: 사용자가 폴더를 선택하면 모든 슬라이드를 `slide_01.png`, `slide_02.png` 형식으로 1920×1080 저장한다.
     - `PDF`: 슬라이드 순서대로 하나의 PDF 파일로 저장한다.
     - `PPTX`: 슬라이드 순서대로 하나의 PPTX 파일로 저장한다.
     - 내보내기가 오래 걸릴 수 있으므로 PNG/PDF/PPTX 모두 진행률 UI를 표시한다. 슬라이드 캡처/추가/저장 루프마다 `현재 / 전체`, `%`, 현재 단계 문구를 갱신하고 막대가 100%까지 차도록 한다.
   - 내보내기 캡처 기준은 좌측 썸네일이 아니라 우측 원본 슬라이드 DOM이며, 캡처 해상도는 1920×1080으로 고정한다.
